...
void predict(Particle *p) {
    ...
    for(int i = 0; i < N; i++){
        ...
        double colision_time = time_to_hit(p, q);

        if(colision_time + t <= limit){
            Event* e1 = create_event(colision_time + t, p, q);
            PQ_insert(pq, e1);
        }
    }
    ...
    //   como nula.
    double colision_vertical = time_to_hit_vertical_wall(p);

    if(t + colision_vertical <= limit){
        Event* e2 = create_event(t + colision_vertical, p, NULL);
        PQ_insert(pq, e2);
    }
    ...
    //   como nula.
    double colision_horizontal = time_to_hit_horizontal_wall(p);

    if(t + colision_horizontal <= limit){
        Event* e3 = create_event(t + colision_horizontal, NULL, p);
        PQ_insert(pq, e3);
    }
}
...
void simulate() {
    ...
    while(!PQ_is_empty(pq) && t <= limit){
        ...
        Particle* a = get_A(next);
        Particle* b = get_B(next);

        for(int i = 0; i < N; i++){
            move_particle(particles[i], get_time(next) - t);
        }
        ...
        if((a != NULL) && (b == NULL)) bounce_off_vertical_wall(a);
        if((a == NULL) && (b != NULL)) bounce_off_horizontal_wall(b);
